...
# move.execute('CREATE TABLE Gtbank (Cust_Id INT(3) PRIMARY KEY AUTO_INCREMENT NOT NULL, Firstname VARCHAR(20) NOT NULL, Surname VARCHAR(20) NOT NULL, Username VARCHAR(20) NOT NULL, Phone INT(11) NOT NULL, Email VARCHAR(30) NOT NULL, Password VARCHAR(20) NOT NULL, Confirm_Password VARCHAR(20) NOT NULL, Pin INT(4) NOT NULL, Confirm_Pin INT(4) NOT NULL, Balance INT(20) NOT NULL, Account_Number INT(10) NOT NULL)')
# move.execute('SHOW TABLES')
# for table in move:
#     print(table)

# move.execute("CREATE TABLE Polaris_Bank (ctm_id INT(4) PRIMARY KEY AUTO_INCREMENT NOT NULL, Firstname VARCHAR(20) NOT NULL, Surname VARCHAR(20) NOT NULL, Username VARCHAR(20) NOT NULL, Phone VARCHAR(11) UNIQUE NOT NULL, Network VARCHAR(11) NOT NULL, Airtime VARCHAR(11) NOT NULL, Data VARCHAR(11) NOT NULL, Email VARCHAR (30) UNIQUE NOT NULL, Password VARCHAR(20) NOT NULL, Confirm_Password VARCHAR(20) NOT NULL, Pin VARCHAR(4) NOT NULL, Confirm_Pin VARCHAR(4) NOT NULL, Balance VARCHAR (20) NOT NULL, Account_Number VARCHAR(10) UNIQUE NOT NULL)")
# move.execute("SHOW TABLES")
# for table in move: 
#     print(table)
class Polaris_Bank:
    ...
    def welcome(self):
        ...
        print(f'Welcome to Polaris Bank')
    ...
    def registration(self):
        ...
        self.account_number = random.randrange(5555555555, 6666666666)   
        print('Have BVN? (Y/N)')
        ...
        if choice == 'Y':
            ...
            if bvnchoice == 1:
                ...
                print('Thank you for your interest in Polaris Bank.\nWe do hope you would check again soon.')
                quit() 
        record = "INSERT INTO Polaris_Bank (Firstname, Surname, Username, Phone, BVN, Airtime, Data, Email, Password, Confirm_Password, Pin, Confirm_Pin, Balance, Account_Number) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    def login(self):
        ...
        while self.logincount <= 3:                   
            ...
            # masking password
            self.pword = maskpass.askpass(prompt = 'Password: ', mask = '*')
            recordlogin = "SELECT * FROM Polaris_Bank WHERE Email = %s AND Password = %s"
    ...
    def menu(self):
        ...
        # Check Balance
        if choice == 1:
            int(maskpass.askpass(prompt = 'Pin: ', mask = '*'))
            # record = "SELECT * FROM polaris_bank WHERE Pin = %s"
            # val = (self.pin)
            # move.execute(record,val)
            # schedule = move.fetchone()
            # print(schedule)
        ...
        # Deposit
            ...
            # move.execute(record, val)
            # schedule = move.fetchone()
            polarisbal = float(self.schedule[-2])
            deposit_amount = float(input('Deposit Amount: '))
            self.balance = polarisbal + deposit_amount
            # self.balance = self.uname
            record = "UPDATE polaris_bank SET  Balance=%s WHERE Pin=%s"
        ...
        # Withdrawal/Transfer
            ...
            if self.pin == self.schedule[-3]:
                ...
                if total_transfer_amount <= self.balance:
                    print(f'1. Transfer within Polaris Bank\n2. Transfer to Other Banks.')
                    choice = int(input('Response: '))
                   
                    # Intra-Bank Transfer - Polaris Bank
                    if choice == 1:
                        ...
                        schedule = move.fetchone()
                        # print(schedule)
                        self.polarisbal = float(schedule[-2])
                        print(f'Name: {schedule[1]} {schedule[2]}, Polaris Bank')
                        # self.pin = int(input('Pin: '))
                        # record1 = "SELECT * FROM polaris_bank WHERE Pin=%s"
                        # val =(self.pin)
                        ...
                        if self.pin == self.schedule[-3]:
                            ...
                            self.newbalance = self.polarisbal + transfer_amount
                            record = "UPDATE polaris_bank SET Balance=%s WHERE Pin=%s"
                            record2 = "UPDATE polaris_bank SET Balance=%s WHERE Account_Number=%s"
                    ...
                    # Transfer to other Bank      
                        ...
                        # Transfer to other bank - GTCO        
                            ...
                            if beneficiary_account_no:
                                ...
                                if self.pin == self.schedule[-3]:   
                                    self.balance -= total_transfer_amount 
                                    self.newbalance = gtcobal + transfer_amount                  
                                    record = "UPDATE polaris_bank SET Balance=%s WHERE Pin=%s"
                                    record1 = "UPDATE gtbank SET Balance=%s WHERE Account_Number=%s"
                                    val1 =(self.balance, self.pin)
                                    val2 = (self.newbalance, beneficiary_account_no)
                                    move.execute(record, val1)
        ...
        # Airtime Purchase
            ...
            #    schedule = move.fetchone()
               # print(schedule)
                print(f'Name: {self.schedule[1]} {self.schedule[2]} {self.schedule[-2]},  Polaris Bank')
                ...
                record = "SELECT * FROM Polaris_Bank WHERE Phone=%s"
                val =(phone_number)
                move.execute(record, val)
                schedule = move.fetchone()
                # print(schedule)
                polarisairtime = float(schedule[6])
                #    print(f'Name: {schedule[1]} {schedule[2]} {schedule[-2]},  GTCO')
                if self.pin == self.schedule[-3]:
                    self.balance -= card_amount
                    self.airtime = polarisairtime + card_amount
                    record = "UPDATE Polaris_Bank SET Balance=%s WHERE Pin=%s"
                    record1 = "UPDATE Polaris_Bank SET Airtime=%s WHERE Phone=%s"
                    val =(self.balance, self.pin)
            ...
            # Data Purchase
                ...
                if data_amount > self.balance:
                    ...
                    schedule = move.fetchone()
                    # print(schedule)
                    polarisdata = float(schedule[7])
                    #    print(f'Name: {schedule[1]} {schedule[2]} {schedule[-2]},  GTCO')
                    if self.pin == self.schedule[-3]:
                        self.balance -= data_amount
                        self.data = polarisdata + data_amount

                        record = "UPDATE Polaris_Bank SET Balance=%s WHERE Pin=%s"
                        record1 = "UPDATE Polaris_Bank SET Data=%s WHERE Phone=%s"
